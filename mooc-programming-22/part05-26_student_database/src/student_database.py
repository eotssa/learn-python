...
if __name__ == "__main__":
    ...
    """
    if course_grade[0] in students[name][0]: #[(0,1), (0,1)]
        print("What is course_grade[0]", course_grade[0])
        print("What is students[name][0]", students[name][0])

    """

    """
    if name in students and course_grade[1] != 0 and course_grade[0] not in students[name]:
        for course in students[name]: 
            if course_grade[0] == course[0]: 
                if course_grade[1] > course[1]:
                    course = course_grade            # THIS DOESN'T WORK BECAUSE COURSE IS A TEMPORARY VARIABLE... 
                    return 

        students[name].append(course_grade)
    """

    #if name in students and (course[0] in student[name]): 
   
    """
    # replace grade if higher
    for course in students[name]: 
        if course_grade[0] in course: # and course_grade[0] > course[0]:
            print("What is course_grade[0]", course_grade[0])
            course = course_grade
    """
