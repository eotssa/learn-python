...
### Reflection
- I like how `shortest()` here is defined, kept it nice and simple.
- I also learned about `lambda` functional programming, and the use of `key=` 
	- Refer [[Useful Functions and Tips (Comphrensive)#`key=` functionality]]
- Recalled differences between `list.pop()` and `list.remove()`
	- `pop()` is index-based
